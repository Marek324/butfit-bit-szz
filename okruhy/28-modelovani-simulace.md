...
- **Spojitá** — funkce Dynamic (vstupy integrátorů) + numerická metoda ([[okruhy/24-numericke-metody|Euler/RK]]) + hlavní cyklus s krokem a **dokročováním**.
- **Kombinovaná** — kombinace + detekce **stavových událostí** (půlení intervalu na přesný okamžik).

Numerické metody viz [[okruhy/24-numericke-metody]]; Monte Carlo / náhodnost viz [[okruhy/31-pravdepodobnost-statistika]].
