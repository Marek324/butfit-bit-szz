...
- **O(n\*d)** časová složitost, kde n je počet prvků v poli a d je počet číslic v největším čísle.
...
- **linearitmická složitost O(n\*logn)**.
...
- časová složitost je v nejhorším případě **kvadratická**, ale vybráním vhodné řady lze snížit na **O(n^(3/2))** nebo **O(n\*(logn)^2)**.
...
Provádí se nad **seřazenou množinou klíčů** s náhodným přístupem (pole). Metoda připomíná metodu **půlení intervalů**. **Složitost** je při nejhorším **logaritmická O(logn)**. Pole **půlíme** a podle **prostřední** **hodnoty** prohledáváme **levou nebo pravou část**. Opět je zde stejný **problém s vkládáním a odebíráním prvků** z pole.
